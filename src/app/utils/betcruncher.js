...
module.exports = (function () {
  ...
  function shouldDoAccumulator(index, type) {
    // If this is the first leg of the bet, but singles are disabled
    // then we should not add to the accumulator
    if (index === 1 && betTypes[type].singles === false) {
      return false;
    }

    // If the total number of selections matches the index of this leg
    // then this accumulator is finished and should be added
    if (betTypes[type].selections === index) {
      return true;
    }

    // If this is an straight accumulator bet, add it
    if (betTypes[type].accumulator === true) {
      return true;
    }
  ...
  function doRound(stake, type, num, isEachWay, runner) {
    ...
    if (isEachWay) {
      eachWayAmounts[num] = doEachWay(
        eachWayAmounts[num - 1],
        runner.odds,
        runner.terms,
        runner.position
      );
    ...
    if (shouldDoAccumulator(num, type) === true) {
      totalStake += stake;
            ...
            for (m = l + 1; m < betTypes[betslip.type].selections; m += 1) {
              ...
              for (n = m + 1; n < betTypes[betslip.type].selections; n += 1) {
                ...
                for (o = n + 1; o < betTypes[betslip.type].selections; o += 1) {
                  ...
                  for (
                    p = o + 1;
                    p < betTypes[betslip.type].selections;
                    p += 1
                  ) {
                    doRound(
                      betslip.stake,
